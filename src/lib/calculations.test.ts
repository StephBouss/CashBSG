...
describe("computeMonthFinancials", () => {
  it("calcule le solde à partir des revenus et des dépenses payées", () => {
    const result = computeMonthFinancials([income(900_000)], [expense(600_000, "paye")]);

    expect(result.totalRevenus).toBe(900_000);
    expect(result.totalDepensesPayees).toBe(600_000);
    expect(result.solde).toBe(900_000 - 600_000);
  });
  ...
  it("ignore les dépenses non payées dans le solde mais les compte à part", () => {
    ...
    expect(result.solde).toBe(500_000 - 200_000);
  });

  it("retourne un solde négatif quand les sorties dépassent les revenus", () => {
    const result = computeMonthFinancials([income(300_000)], [expense(320_000, "paye")]);

    expect(result.solde).toBe(300_000 - 320_000);
